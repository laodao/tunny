...
function parseUrl(url, method){
    ...
    if(controllers.get(reqPath)){
        ...
        return {'form':form, 'controller':controller};
    }else{
        ...
        for(var i=0;i<pathPart.length;i++){
            ...
            if(!path.existsSync(fullPath)){//动态请求没有后缀，没找到的时候就去寻找文件名为path+.js的文件，否则就视为找到一个目录
                ...
                if(path.existsSync(fullPath)){
                    ...
                    var params = paramStr.split('/');
                    var controller = new Controller(actionName, fullPath, controllerPath);
                    var interceptor = new Interceptor(controller);
                    controller.setInterceptor(interceptor);
                    if(config.env=='dev'){
...
exports.parseSocketReq = function(_path){
    var fullPath = path.join(config.baseDir, config.appPath, _path);
    var controller = new Controller(actionName, fullPath, _path);
    var interceptor = new Interceptor(controller);
    controller.setInterceptor(interceptor);
    var webForm = new WebForm(controller);
    return {'form':webForm, 'controller':controller};
};
