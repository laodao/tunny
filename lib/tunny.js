...
exports.createServer = function(port) {
	...
	var server = http.createServer(function(req, res) {
		...
		var suffix = path.extname(parseURL(url, true).pathname).toLowerCase();
		...
		route.parseReq(req, function(reqParser){
            if(!reqParser){
                httpHandler.handler404(res);
                return;
            }
            ...
            var sessionID;
            var _cookie = appUtil.parseCookie(req.headers.cookie, function(cookie){
                sessionID = _cookie.sessionID;
                webForm.addCookie( "sessionID",sessionID);
                //session与cookie处理
                var session = sessionMgr.getSession(sessionID);
                session.once("_ready", function(_session){//session初始化事件
	...
	function initWebSocket(io){
		...
		ios.forEach(function(ioService){
			...
			ioService.getIo().on('connection', function(socket) {
				...
				//sc.addClient(socket);
				function handlerEvent(conf){//事件处理类，封装webform，然后调用action，最后返回给客户端
                    ...
                    var instance = this;
                    route.parseSocketReq(conf, function(parserObj){
                        ...
                        socket.on(conf.event, function(data){
...
//                                        }
				...
				ioInstance.getSocketConf().forEach(function(conf){
                    ...
                    handlerEvent(conf);
					//socket.on(conf.event, (eventHandler(conf))(conf));
					//}
				});
