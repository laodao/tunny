...
//    var action = require(controllerPath);
    ...
    if(!interceptor){
        interceptor = {
            before:function(form, beforeInterceptor){
                logger.info('no interceptor');
                beforeInterceptor(form, beforeRender);
            },
            after:function(form, afterInterceptor){
                afterInterceptor(form, beforeRender);
            }
        };
    }
...
//        logger.info('start call the action method:'+fn);
        console.log('---------------');
        handler(webForm, render);//执行action方法
    }
...
var forward = function (req, res, newUrl, webForm) {
    route.getController(newUrl, function(reqParser){
        if (reqParser&&reqParser.action) {
...
//        webForm.setReq(req);
            invokeAction(req, res, reqParser.action, webForm, send);
        } else {
